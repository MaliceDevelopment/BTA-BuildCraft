...
public final class PowerHandler {
    ...
    private void updateSources(Direction source) {
        if (sourcesTracker.markTimeIfDelay(receptor.getWorld(), 1)) {
            for (int i = 0; i < 6; ++i) {
                powerSources[i] -= (int) sourcesTracker.durationOfLastDelay();
                if (powerSources[i] < 0) {
                    powerSources[i] = 0;
                }
            }
    ...
    public void readFromNBT(NbtCompound data, String tag) {
        NbtCompound nbt = data.getCompound(tag);
    ...
    public void writeToNBT(NbtCompound data, String tag) {
    ...
    public final class PowerReceiver {
         ...
         */
        public double receiveEnergy(Type source, final double quantity, Direction from) {
    ...
    public boolean isPowerSource(Direction from) {
